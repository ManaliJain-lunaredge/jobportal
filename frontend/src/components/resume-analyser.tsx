...
import { ResumeAnalysisResponse, Suggestion } from "@/type";
...
{Object.entries(response.scoreBreakdown).map(([key, value]) => {
	const v = value as { score: number; feedback: string };
	return (
		<div key={key} className="p-4 rounded-lg border">
			<div className="flex items-center justify-between mb-2">
				<p className="font-semibold capitalize">{key}</p>
				<span className={`text-lg font-bold ${getScoreColor(v.score)}`}>
					{v.score}%
				</span>
			</div>
			<p className="text-xs opacity-70">{v.feedback}</p>
		</div>
	);
})}
</div>
